/**
 * 🏛️🖥️ Organization Type Systems Dialog Component
 * (/[locale]/(main)/admin/organization-type/actions/system.tsx)
 * 
 * Энэ нь байгууллагын төрөлд систем оноох dialog юм.
 * Зорилго: Тухайн төрлийн байгууллагууд ямар системүүдэд хандах эрхтэй болохыг тохируулах
 * 
 * Features:
 ...
 * 
 * Workflow:
 * 1. Open dialog → Load all systems + assigned systems
 * 2. Display card grid with checkboxes
 * 3. Search/filter available
 * 4. Multi-select systems
 ...
 * 
 * API Endpoints:
 * - GET /system?page=1&size=500 - Get all systems
 * - GET /orgtype/system?type_id=... - Get assigned systems
 * - POST /orgtype/system { type_id, system_ids: [] } - Save assignments
 * 
 * Data Flow:
 * - Load all systems (500 max)
 * - Load assigned system IDs
...
type SystemRow = App.System

export default function OrgTypeSystemsDialog({ open, onOpenChange, orgType }: Props) {
  ...
  const [systems, setSystems] = React.useState<SystemRow[]>([])
  const [selectedIds, setSelectedIds] = React.useState<Set<number>>(new Set())

  const [q, setQ] = React.useState('')

  /**
   * 📥 Бүх системүүдийг ачаалах
   */
  const fetchAllSystems = React.useCallback(async () => {
    const data = await api.get<App.ListData<SystemRow> | SystemRow[]>('/system', {
  ...
  /**
   * 📥 Байгууллагын төрөлд оноогдсон системүүдийг ачаалах
   */
  const fetchSelectedSystems = React.useCallback(async (orgTypeId: number) => {
    const data = await api.get<App.System[]>('/orgtype/system', {
      query: { type_id: orgTypeId, page_size: 500 },
    })
    const ids = (data as App.System[]).map((x) => x.id)
    ...
    ;(async () => {
      ...
      try {
        const [all, selected] = await Promise.all([
          fetchAllSystems(),
          fetchSelectedSystems(orgType.id),
        ])
        setSystems(all)
  ...
  }, [open, orgType?.id, fetchAllSystems, fetchSelectedSystems])
  ...
  /**
   * ☑️ Системийг сонгох/цуцлах toggle
   */
  ...
  /**
   * ☑️ Бүх харагдаж буй системүүдийг сонгох/цуцлах
   */
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-4xl">
        <DialogHeader>
          <DialogTitle>
            {t('system')} — <span className="font-medium">{orgType?.name ?? ''}</span>
          </DialogTitle>
        </DialogHeader>
                    ...
                    <div className="flex items-start gap-3 p-4">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {s.code ? (
                            <span className="bg-muted rounded-md px-1.5 py-0.5 text-[11px] leading-none uppercase">
                              {s.code}
                            </span>
                          ) : null}
                        </div>
